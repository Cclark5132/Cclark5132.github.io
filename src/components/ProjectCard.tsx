...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    ...
    >
      <Link to={`/projects/${project.slug}`} className="flex h-full flex-col p-6 md:p-7" aria-label={`View ${project.title} case study`}>
        ...
        <h3 className="mt-6 text-2xl font-semibold tracking-[-0.035em] text-charcoal md:text-3xl">{project.title}</h3>
        <p className="mt-3 text-base leading-7 text-steel">{project.subtitle}</p>
        <div className="mt-6 flex items-end justify-between gap-5 border-t border-line pt-5">
          <div>
